...
function ItemButton({ item, onAdd }) {
  return (
    <button
      ...
      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left hover:bg-primary hover:border-amber-200 border border-transparent transition-all group"
    >
      ...
      <span className="ml-auto text-gray-300 group-hover:text-amber-400 text-lg shrink-0">
        +
      </span>
    </button>
  );
...
export default function CatalogSidebar() {
  ...
  const suggestionIds = !isSearching
    ? categorySuggestions[activeCategory]
    : null;
  const suggested = suggestionIds
    ? furnitureCatalog.filter(
        (item) =>
          suggestionIds.includes(item.id) && item.category !== activeCategory,
      )
    : [];
