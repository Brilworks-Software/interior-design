...
export default function ViewportControls() {
  ...
  const showProductDimensions = useDesignerStore(
    (s) => s.showProductDimensions,
  );
  const measureUnit = useDesignerStore((s) => s.measureUnit);
  const toggleRoomDimensions = useDesignerStore((s) => s.toggleRoomDimensions);
  const toggleProductDimensions = useDesignerStore(
    (s) => s.toggleProductDimensions,
  );
  ...
  return (
    ...
    >
      ...
      {openPanel === "room" && (
        <RoomCustomPanel onClose={() => setOpenPanel(null)} />
      )}
            ...
            {sideViews.map((v) => (
              <button
                ...
                className={`py-1.5 px-2 rounded-lg text-xs font-medium border transition-all ${
                  viewMode === v.id
                    ? "bg-primary text-white border-primary0 shadow-sm"
        ...
        <div className="absolute bottom-12 right-0 w-60 bg-white rounded-xl shadow-xl border border-gray-100 p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <Toggle
              ...
              onChange={() =>
                setLightingMode(lightingMode === "off" ? "bright" : "off")
              }
            />
          </div>
      ...
      {openPanel === "measure" && (
        <div className="absolute bottom-12 right-0 w-64 bg-white rounded-xl shadow-xl border border-gray-100 p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <Toggle
              checked={showProductDimensions}
              onChange={toggleProductDimensions}
            />
          </div>
      ...
      <div className="flex gap-1 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg px-1 py-1 shadow-lg">
        ...
        >
          Grid
          {gridSnap
            ? ` · ${gridSize === 0.25 ? "¼" : gridSize === 0.5 ? "½" : "1"} ft`
            : ""}
        </button>
        ...
        <button
          ...
          title="Lighting Mood"
          className={`px-3 py-1.5 text-xs font-medium rounded transition-all ${
            openPanel === "lighting" || lightingMode !== "off"
              ? "bg-primary text-white shadow-sm"
              : "text-gray-600 hover:bg-gray-100"
          }`}
        >
