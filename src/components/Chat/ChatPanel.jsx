  ...
  if (!open) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-5 py-3 bg-primary hover:bg-secondary text-white font-medium rounded-full shadow-lg hover:shadow-xl transition-all"
      >
        <Sparkles size={18} />
        Design with AI
      </button>
    ...
    <div className="fixed bottom-6 right-6 z-50 w-[420px] max-h-[600px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
        ...
        <div className="px-4 pb-2 flex flex-wrap gap-1.5">
          {SUGGESTIONS.map((s, i) => (
            <button
              ...
              className="text-xs px-2.5 py-1.5 bg-primary text-amber-700 border border-amber-200 rounded-lg hover:bg-amber-100 transition-colors text-left"
            >
              {s}
            </button>
          ))}
      ...
      <div className="border-t border-gray-200 p-3 flex gap-2">
        ...
        <button
          ...
          className="px-3 py-2 bg-primary text-white rounded-lg hover:bg-secondary disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          <Send size={16} />
        </button>
      </div>
