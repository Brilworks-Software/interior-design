...
const DOOR_STYLES = [
  ...
  { id: "glass", name: "Glass Door", width: 3, Svg: GlassDoorSvg },
  {
    ...
    width: 5,
    Svg: FrenchDoubleDoorSvg,
  },
  {
    id: "double",
    name: "Double Panel Door",
    width: 5,
    Svg: DoublePanelDoorSvg,
  },
    ...
    <div className={`${color} rounded p-2 space-y-1.5`}>
      ...
      {type === "door" && wall.door && (
        <>
          <SliderField
            label="Position"
            value={wall.door.position}
            min={0.15}
            max={0.85}
            step={0.05}
            unit=""
            onChange={(v) =>
              onUpdate({ ...wall, door: { ...wall.door, position: v } })
            }
          />
          <SliderField
            label="Width"
            value={wall.door.width}
            min={2}
            max={wallLen * 0.5}
            step={0.5}
            unit=" ft"
            onChange={(v) =>
              onUpdate({ ...wall, door: { ...wall.door, width: v } })
            }
          />
        </>
      )}
      {type === "window" && wall.window && (
        <>
          <SliderField
            label="Position"
            value={wall.window.position}
            min={0.15}
            max={0.85}
            step={0.05}
            unit=""
            onChange={(v) =>
              onUpdate({ ...wall, window: { ...wall.window, position: v } })
            }
          />
          <SliderField
            label="Width"
            value={wall.window.width}
            min={2}
            max={wallLen * 0.8}
            step={0.5}
            unit=" ft"
            onChange={(v) =>
              onUpdate({ ...wall, window: { ...wall.window, width: v } })
            }
          />
          <SliderField
            label="Height"
            value={wall.window.height}
            min={2}
            max={roomHeight * 0.6}
            step={0.5}
            unit=" ft"
            onChange={(v) =>
              onUpdate({ ...wall, window: { ...wall.window, height: v } })
            }
          />
          <SliderField
            label="Elevation"
            value={wall.window.elevation}
            min={1}
            max={roomHeight - wall.window.height - 1}
            step={0.5}
            unit=" ft"
            onChange={(v) =>
              onUpdate({ ...wall, window: { ...wall.window, elevation: v } })
            }
          />
        </>
      )}
...
export default function DoorsWindowsStep({
  walls,
  width,
  depth,
  height,
  ...
  function addDoor(styleId) {
    ...
    onUpdateWall(activeWall, {
      ...rest,
      door: { position: 0.5, width: dw, style: styleId },
    });
  }
    ...
    <div className="space-y-3 pb-2">
      ...
      <div>
        ...
        <div className="flex items-center justify-between mt-2">
          <span className="text-[10px] text-gray-500">
            {WALL_LABELS[activeWall]} wall &middot; {wallLen} ft
          </span>
          <button
            ...
            <div className="space-y-1.5">
              {wall.door && (
                <PlacedItem
                  type="door"
                  label={
                    DOOR_STYLES.find((s) => s.id === wall.door.style)?.name ||
                    "Door"
                  }
                  color="bg-primary text-amber-800"
                  wall={wall}
                  wallLen={wallLen}
                  roomHeight={height}
              ...
              {wall.window && (
                <PlacedItem
                  type="window"
                  label={
                    WINDOW_STYLES.find((s) => s.id === wall.window.style)
                      ?.name || "Window"
                  }
      ...
      {!wall.exists && (
        <p className="text-xs text-gray-400 text-center py-4">
          This wall is turned off. Toggle it on to add doors or windows.
        </p>
      )}
    </div>
  );
